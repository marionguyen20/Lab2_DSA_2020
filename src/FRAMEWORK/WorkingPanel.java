...
//
        ...
        if(this.pointList != null){
            for(int idx=0; idx < this.pointList.length; idx++)
                this.pointList[idx].draw(g, spaceMapping);
        }
        if(this.graph != null)
            this.graph.draw(g, spaceMapping);
    }
    ...
    public void actionPerformed(ActionEvent e) {
        if(e.getSource() == MainFrame.btnCircle){
            ...
            Graph graph;
            graph = Graph.sin(50, -10, 10,this.spaceMapping.step());
            graph.setMode(Graph.GraphMode.SCATTER);
            this.axis.addGraph(graph,
                    Color.red, 2);
            //
            this.axis.addGraph(Graph.quadratic(1, 0, 0, -10, 10, this.spaceMapping.step()),
                    Color.blue, 3);
            //
            graph = new Graph(Point2D.linear(100, 1.0, 0.0, -10, 10));
            this.axis.addGraph(graph, Color.cyan, 1);
