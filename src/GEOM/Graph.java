...
public class Graph extends GeomObject {
    ...
    //Setter and getter for mod

    public GraphMode getMode() {
        return mode;
    }
    public void setMode (GraphMode mode) {this.mode = mode;}
    ...
    public void draw(Graphics g, SpaceMapping mapper) {
       ...
       //
        if (this.mode == GraphMode.LINE) {
            ...
            Stroke stroke = new BasicStroke(this.line_weight);
            g2.setStroke(stroke);
    ...
    public static Graph sin (double a, double xMin, double xMax, double step) {
        ...
        for (int idx = 0; idx < N; idx ++) {
            double y = a*Math.sin(2*Math.PI*(1.0/T)*(x - xMin)); //Draw with 1 period
